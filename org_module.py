...
USERS_PASS = st.secrets["user_password"]
PM_PASS = st.secrets["pm_password"]
...
PM = config_handler.get_config_values('constants', 'PM')
USERS = config_handler.get_config_values('constants', 'USERS')
...
MIN_PROFILES = config_handler.get_config_values('menu_lists', 'MIN_PROFILES')
GOVT_ORGS = config_handler.get_config_values('menu_lists', 'GOVT_ORGS')
...
def display_accounts(school_id):
	...
	columns = ["User ID", "Username", "Profile", "Statutory Board", "Division", "Branch", "Ministry"]
...
def create_org_structure():
	with sqlite3.connect(WORKING_DATABASE) as conn:
		...
		org_name = st.selectbox("Select a ministry:", org_names)

		# Collect user inputs
		school_name = st.text_input("Enter a statutory board:")
		num_levels = st.number_input("Enter number of division in the stat board:", min_value=1, value=1)

		level_names = [st.text_input(f"Enter name for division {i + 1}:") for i in range(num_levels)]
		num_classes_per_level = [st.number_input(f"Enter number of branches for division {i + 1}:", min_value=1, value=1) for i in range(num_levels)]
		class_names = [[st.text_input(f"Enter name for branch {j + 1} in branch {i + 1}:") for j in range(num_classes_per_level[i])] for i in range(num_levels)]
		num_students_per_class = [[st.number_input(f"Enter number of users for branch {j + 1} in division {i + 1}:", min_value=1, value=1) for j in range(num_classes_per_level[i])] for i in range(num_levels)]
		
		num_teachers = st.number_input("Enter number of managers for the statboard:", min_value=1, value=1)
		...
		if st.button('Submit'):
			...
			if existing_school:
				st.warning(f"Statboard {school_name} already exists. Please choose a different name")
			else:
				...
				for i, level_name in enumerate(level_names):
					...
					for j, class_name in enumerate(class_names[i]):
						...
						for k in range(num_students_per_class[i][j]):
							student_username = f"user{school_id}_{student_counter}"
							cursor.execute("INSERT INTO Users (username, password, profile_id, school_id, class_id, org_id, level_id) VALUES (?, ?, ?, ?, (SELECT class_id FROM Classes WHERE class_name = ?), (SELECT org_id FROM Organizations WHERE org_name = ?), (SELECT level_id FROM Levels WHERE level_name = ?))", (student_username, hash_password(USERS_PASS),USERS, school_id, class_name, org_name, level_name))
							student_counter += 1
				for t in range(num_teachers):
					teacher_username = f"pm{school_id}_{t + 1}"
					cursor.execute("INSERT INTO Users (username, password, profile_id, school_id, org_id) VALUES (?, ?, ?, ?, (SELECT org_id FROM Organizations WHERE org_name = ?))", (teacher_username, hash_password(PM_PASS), PM, school_id, org_name))
...
def remove_or_reassign_teacher_ui(school_id):
	...
	st.title("Assign, Remove, or Reassign managers to/from Branches")

	with sqlite3.connect(WORKING_DATABASE) as conn:
		cursor = conn.cursor()

		show_unassigned = st.checkbox("Show unassigned managers")

		if show_unassigned:
			# Fetch managers not assigned to any branch from the statboard
			cursor.execute("""
			...
			selected_teacher = st.selectbox("Select unassigned managers:", unassigned_teachers)
		else:
			...
			selected_teacher = st.selectbox("Select managers to Remove or Reassign:", assigned_teachers)
			...
			if selected_teacher:
				...
				st.write(f"Branches currently assigned to {selected_teacher}: {', '.join(assigned_classes_to_teacher)}")

		remove_assignment = st.checkbox("Remove branches from all classes")
		...
		if not remove_assignment:
			...
			selected_classes = st.multiselect("Select branches (For Reassignment or Assignment):", all_classes, default=all_classes[0])

		st.markdown("---")
		btn_process = st.button("Process Managers")
		...
		if btn_process:
			...
			conn.commit()
			st.success(f"Manager {selected_teacher} has been processed successfully!")
...
def change_teacher_profile_ui(school_id):
    ...
    """
    st.title("Change Manager Profiles")

    with sqlite3.connect(WORKING_DATABASE) as conn:
        cursor = conn.cursor()

        # Fetch all profile names and their IDs excluding the 'USERS' profile
        cursor.execute("SELECT profile_id, profile_name FROM Profile WHERE profile_id != ? AND profile_id != ?", (USERS,SA))
        ...
        # Get profile_ids that match the MIN_PROFILES list
        sch_profile_ids = [profile_name_to_id[profile_name] for profile_name in MIN_PROFILES if profile_name in profile_name_to_id]

        # Fetch all teachers within the given school that have a profile in MIN_PROFILES
        cursor.execute("""
        ...
        if not teachers:
            st.write("No manager found for this school!")
            return
        ...
        if st.button("Update Profiles"):
            ...
            conn.commit()
            st.success("Manager profiles updated successfully!")
...
def reassign_student_ui(school_id):
	...
	:param WORKING_DATABASE: Path to the SQLite database file.
	:param USERS: Global variable for student's profile_id.
	:param school_id: The ID of the school to which students are to be reassigned.
	"""

	st.title("Reassign Users to Another Branch")
	...
	with sqlite3.connect(WORKING_DATABASE) as conn:
		...
		selected_level = st.selectbox("Select Division:", all_levels)
		...
		selected_class = st.selectbox("Select Branch:", all_classes_in_level)

		# Fetching students from the selected class and level
		cursor.execute("SELECT username FROM Users WHERE profile_id = ? AND school_id = ? AND class_id IN (SELECT class_id FROM Classes WHERE class_name = ?)", (USERS, school_id, selected_class))
		students_in_class = [student[0] for student in cursor.fetchall()]
		selected_students = st.multiselect("Select Users to Reassign:", students_in_class)
		...
		new_class = st.selectbox("Select New Branch for Reassignment:", all_classes_for_reassignment)
		...
		if btn_reassign:
			...
			conn.commit()
			st.success(f"Selected users have been reassigned to {new_class} successfully!")
...
def process_user_profile(profile_name):
	...
	with sqlite3.connect(WORKING_DATABASE) as conn:
		...
		if profile_name == SA:   # Using the global SA constant
			...
			org_name_selected = st.selectbox("Choose a Ministry:", [org[0] for org in all_organization_names])
			...
			if not schools_in_org:
				return None, "No statutory board found for the selected ministry. Please create a statutory board first."

			school_selected = st.selectbox("Choose a Statutory Board within the Ministry:", [school[0] for school in schools_in_org])
			...
			return school_id, f"Selected Statutory Board ID: {school_id} within Ministry: {org_name_selected}"
		else:   # Using the global AD constant
			return st.session_state.user["school_id"], f"User's Statutory Board ID: {st.session_state.user['school_id']}"
...
def add_level(school_id):
    with sqlite3.connect(WORKING_DATABASE) as conn:
        ...
        # Collect inputs
        level_name = st.text_input("Enter Division Name:")
        ...
        if st.button('Add Division'):
            ...
            if existing_level:
                st.warning(f"Division {level_name} already exists for this Statutory Board. Please choose a different name.")
            else:
                cursor.execute("INSERT INTO Levels (org_id, school_id, level_name) VALUES (?, ?, ?)", (org_id, school_id, level_name))
                conn.commit()
                st.success("Division added successfully!")
...
def add_class(school_id):
    with sqlite3.connect(WORKING_DATABASE) as conn:
        ...
        # Collect inputs
        level_name = st.selectbox("Select Division:", available_levels)
        class_name = st.text_input("Enter Branch name:")
...
def add_user(school_id):
    with sqlite3.connect(WORKING_DATABASE) as conn:
        ...
        user_type = st.selectbox("Select user type:", ["Managers", "Users"])
        ...
        if user_type == "Managers":
            if st.button(f'Add {user_type}'):
                ...
                cursor.execute("INSERT INTO Users (username, password, profile_id, school_id, org_id) VALUES (?, ?, ?, ?, ?)", (teacher_username, hash_password(PM_PASS), PM, school_id, org_id))
                conn.commit()
            ...
            if st.button(f'Add {user_type}'):
                ...
                cursor.execute("INSERT INTO Users (username, password, profile_id, school_id, class_id, org_id, level_id) VALUES (?, ?, ?, ?, (SELECT class_id FROM Classes WHERE class_name = ? AND school_id = ?), ?, (SELECT level_id FROM Levels WHERE level_name = (SELECT level_name FROM Classes WHERE class_name = ? AND school_id = ?) AND school_id = ?))", 
                               (student_username, hash_password(USERS_PASS), USERS, school_id, class_name, school_id, org_id, class_name, school_id, school_id))
                conn.commit()
                st.success(f"{user_type} added successfully!")
...
def delete_class(class_id):
    with sqlite3.connect(WORKING_DATABASE) as conn:
        ...
        return "Branch Deleted Successfully!" if success else "Cannot delete: Branch has associated records."

def delete_level(level_id):
    with sqlite3.connect(WORKING_DATABASE) as conn:
        cursor = conn.cursor()
        success = delete_if_no_association(cursor, "Levels", "level_id", level_id)
        return "Division Deleted Successfully!" if success else "Cannot delete: Division has associated records."

def delete_school(school_id):
    with sqlite3.connect(WORKING_DATABASE) as conn:
        cursor = conn.cursor()
        success = delete_if_no_association(cursor, "Schools", "school_id", school_id)
        return "Statutory Board Deleted Successfully!" if success else "Cannot delete: Statutory Board has associated records."
...
def streamlit_delete_interface():
    ...
    choice = st.selectbox("Which entity do you want to delete?", ["Branch", "Division", "Satutory Board"])
    
    if choice == "Branch":
        class_ids = get_values_from_table("Classes", "class_id")
        selected_class = st.selectbox("Choose a Branch to delete:", class_ids)
        ...
        selected_level = st.selectbox("Choose a Division to delete:", level_ids)
        if st.button("Delete Division"):
            result = delete_level(selected_level)
            st.write(result)

    elif choice == "Statutory Board":
        school_ids = get_values_from_table("Schools", "school_id")
        selected_school = st.selectbox("Choose a Statutory Board to delete:", school_ids)
        if st.button("Delete Statutory Board"):
